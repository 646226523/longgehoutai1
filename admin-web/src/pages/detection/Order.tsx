    ...
    <>
      ...
      <Drawer
        ...
        footer={(() => {
          ...
          if (!getFormString('ring_number')) missing.push('足环号');
          if (!getFormString('user_name')) missing.push('预约人姓名');
          if (!formValues.org_id) missing.push('检测机构');
          const p = formValues.project;
          const pList = Array.isArray(p) ? (p as string[]) : p ? [p as string] : [];
          if (pList.length === 0) missing.push('检测项目（至少选一项）');
            ...
            if (disabled) {
              Modal.warning({
                ...
                content: (
                  <div>
                    <div style={{ marginBottom: 8, color: '#666' }}>
                      还有 {missing.length} 项未填写完整：
                    </div>
                    <ul style={{ paddingLeft: 20, color: '#ff4d4f', marginBottom: 0 }}>
                      {missing.map((m) => (
                        <li key={m} style={{ lineHeight: '24px' }}>{m}</li>
                      ))}
                    </ul>
                  </div>
                ),
                ...
                onOk: () => {
                  ...
                  const fieldName = nameMap[first];
                  if (fieldName) {
                    const el = document.querySelector(
                      `[id$="${fieldName}"] input, [data-field-name="${fieldName}"] input`
                    ) as HTMLElement | null;
                    el?.focus?.();
          ...
          return (
            <div style={{ textAlign: 'right', display: 'flex', gap: 8, justifyContent: 'flex-end', alignItems: 'center' }}>
              {/* 缺失项提示 chip */}
              {disabled && (
                <span style={{ fontSize: 12, color: '#ff4d4f', marginRight: 'auto' }}>
                  缺失 {missing.length} 项：{missing.slice(0, 2).join('、')}{missing.length > 2 ? '...' : ''}
                </span>
              )}
              {!disabled && (
                <span style={{ fontSize: 12, color: '#52c41a', marginRight: 'auto', display: 'flex', alignItems: 'center', gap: 4 }}>
                  <CheckCircleFilled /> 所有必填项已完成
                </span>
              )}
              <Button
                onClick={() => {
              ...
              <Button type="primary" disabled={disabled} onClick={handleSubmitClick}>
                {editing ? '保存修改' : '确认提交'}
              </Button>
            </div>
          );
            ...
            >
              ...
              >
                <Checkbox.Group
                  ...
                  value={(() => {
                    const p = formValues['project'];
                    return (Array.isArray(p) ? p : p ? [p] : []) as string[];
                  })()}
                  onChange={(checkedValues) => {
                    const arr = checkedValues as string[];
                    formRef.current?.setFieldsValue({ project: arr });
                    setFormValues((prev) => ({ ...prev, project: arr }));
                    if (arr.length > 0) setCurrentStep(2);
                  }}
                >
                  <Row gutter={[12, 12]}>
                    {itemTypes.length > 0 ? (
